...
def _hdbscan_cluster(reduced: np.ndarray, min_cluster_size: int) -> Tuple[np.ndarray, float]:
    ...
    logger.info("HDBSCAN produced %d clusters with DBCV score %.3f.", n_clusters, validity)
    ...
    *,
    ...
    try:
        ...
        if sum(valid_indices) > 2 and len(set(labels[valid_indices])) > 1:
            try:
                cluster_accuracy = float(silhouette_score(embeddings[valid_indices], labels[valid_indices]))
            except Exception:
                pass
