...
def generate_pdf_report(
    ...
    try:
        ...
        story.append(Paragraph(f"<b>Topic:</b> {escape(str(topic or 'N/A'))}", body_style))
        ...
        #Pattern Analysis
        story.append(Paragraph("1. Pattern Analysis", h2_style))
        
        # Safe access to pattern attributes
        top_m = (getattr(pattern, 'top_methods', []) or [])[:5]
        top_d = (getattr(pattern, 'top_datasets', []) or [])[:5]
        ...
        def wrap_p(text, bold=False, is_html=False):
            style = ParagraphStyle("TableCell", parent=styles["Normal"], fontSize=9, leading=13)
            if bold:
        ...
        pat_data = [
            ...
            [wrap_p("Baseline Stagnation"), wrap_p("Potential stagnation detected" if getattr(pattern, 'baseline_stagnation', False) else "Innovation pace remains steady")],
        ]
        
        story.append(Paragraph("1. High-Level Pattern Analysis", h2_style))
        story.append(Paragraph("This section synthesizes recurring themes, methodological trends, and dataset usage observed across the analyzed research corpus.", body_style))
        ...
        if opps:
            story.append(Paragraph("Emerging Opportunities", h2_style))
            for opp in opps:
                story.append(Paragraph(f"• {escape(str(opp))}", body_style))
            story.append(Spacer(1, 0.4 * cm))

        #Research Gaps
        story.append(Paragraph("2. Critical Research Gaps", h2_style))
        if not gaps:
            story.append(Paragraph("No significant research gaps were identified in the analyzed corpus.", body_style))
        else:
            for i, gap in enumerate(gaps, 1):
                story.append(Paragraph(f"{i}. {escape(str(gap.gap_title or 'Unnamed Gap'))}", ParagraphStyle("GapTitle", parent=styles["Heading3"], fontSize=12, textColor=colors.HexColor("#4f46e5"), spaceBefore=10)))
                raw_score = getattr(gap, 'confidence_score', 0.0)
                score = float(raw_score) if raw_score is not None else 0.0
                story.append(Paragraph(f"<b>Confidence:</b> {score:.2f} | <b>Cluster Reference:</b> #{getattr(gap, 'cluster_id', 'N/A')}", label_style))
                
                # Grouped Gap Content
                story.append(Paragraph(f"<b>Impact Statement:</b> {escape(str(gap.description))}", body_style))
                story.append(Paragraph(f"<b>Technical Constraint:</b> {escape(str(gap.what_fails))}", body_style))
                story.append(Paragraph(f"<b>Origin of Gap:</b> {escape(str(gap.why_it_exists))}", body_style))
                story.append(Paragraph(f"<b>Missing Component:</b> {escape(str(gap.missing_piece))}", body_style))
                story.append(Paragraph(f"<b>Detected Pattern:</b> {escape(str(gap.pattern_detected))}", body_style))
                story.append(Paragraph(f"<b>Proposed Trajectory:</b> {escape(str(gap.proposed_direction))}", body_style))
                
                # Citations
                cites = getattr(gap, 'citations', [])
                if cites:
                    story.append(Paragraph("<b>Primary Source References:</b>", label_style))
                    for idx, c in enumerate(cites, 1):
                        title_text = escape(str(c.title))
                        if getattr(c, "url", None):
                            # ReportLab Link syntax
        ...
        story.append(Paragraph("3. Synthesis Recommendations", h2_style))
        if gaps:
            # Safer access to top gap
            top_gap = gaps[0]
            rec_text = getattr(top_gap, 'proposed_direction', "Explore identified clusters for novel research paths.")
            story.append(Paragraph(f"<b>Primary Research Trajectory:</b> {escape(str(rec_text))}", body_style))
        else:
            story.append(Paragraph("Based on the current analysis, we recommend expanding the search parameters to identify more niche research clusters.", body_style))

        #Visualizations
        story.append(Spacer(1, 1 * cm))
        story.append(Paragraph("4. Analytical Visualizations", h2_style))
        story.append(Paragraph("The following charts provide a visual decomposition of the research landscape based on semantic embedding and meta-data analysis.", body_style))

        chart_fields = [
            ("umap_scatter", "Semantic Cluster Mapping"),
            ("confidence_bars", "Gap Confidence Distribution"),
            ("year_distribution", "Temporal Publication Density"),
            ("method_trends", "Methodological Evolution"),
        ...
        for field, chart_title in chart_fields:
            b64 = None
            if hasattr(visualizations, field):
                b64 = getattr(visualizations, field)
            elif isinstance(visualizations, dict):
                b64 = visualizations.get(field)
            
            img_bytes = _decode_b64_image(b64)
            if img_bytes:
                try:
                    story.append(Paragraph(chart_title, ParagraphStyle("ChartT", parent=styles["Normal"], fontSize=11, fontWeight='bold', spaceBefore=15, spaceAfter=8)))
        ...
        if not found_viz:
            story.append(Paragraph("Visual analytics are currently being generated or were unavailable for this report corpus.", body_style))
...
def _minimal_pdf(topic: str, report_id: str, gaps: list) -> bytes:
    """Bare-bones PDF when ReportLab is unavailable or fails."""
    lines = [
    ...
    for i, gap in enumerate(gaps, 1):
        lines.append(f"{i}. {gap.gap_title} (confidence: {gap.confidence_score:.2f})")
        lines.append(f"   {gap.description}")
    content = "\n".join(lines)
    # Minimal valid PDF 1.4 with embedded text
    text_escaped = content.replace("\\", "\\\\").replace("(", "\\(").replace(")", "\\)")
    
    # We must draw each line separately in PDF stream syntax because \n doesn't work inside Tj.
