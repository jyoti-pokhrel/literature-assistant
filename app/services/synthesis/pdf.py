...
def generate_pdf_report(
    ...
    try:
        ...
        meta_style = ParagraphStyle("Meta", parent=styles["BodyText"], fontSize=9, textColor=colors.HexColor("#64748b"))

        def wrap_p(text, bold=False, is_html=False):
            style = ParagraphStyle("TC", parent=styles["Normal"], fontSize=9, leading=13)
            if bold:
        ...
        if gaps:
            ...
            story.append(Paragraph(
                ...
                "Consider expanding search parameters or adjusting year/venue filters.", body_style))
        ...
        pat_data = [
            ...
            [wrap_p("Baseline Stagnation"), wrap_p(
                "Potential stagnation detected" if getattr(pattern, 'baseline_stagnation', False) else "Innovation pace remains steady"
            )],
        ]
        ...
        if not gaps:
            ...
            for i, gap in enumerate(gaps, 1):
                story.append(Paragraph(f"{i}. {escape(str(gap.gap_title or 'Unnamed Gap'))}", h3_style))
        ...
        for field, chart_title in chart_fields:
            b64 = visualizations.get(field) if isinstance(visualizations, dict) else getattr(visualizations, field, None)
            img_bytes = _decode_b64_image(b64)
            if img_bytes:
                try:
                    story.append(Paragraph(chart_title, ParagraphStyle("ChartT", parent=styles["Normal"], fontSize=11, fontName="Helvetica-Bold", spaceBefore=15, spaceAfter=8)))
