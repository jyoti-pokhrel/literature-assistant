...
def make_confidence_bars(gaps: list) -> Optional[str]:
    ...
    try:
        ...
        colors = [
            "#6366f1" if s >= 0.8 else "#a78bfa" if s >= 0.6 else "#c4b5fd"
            for s in scores
        ]
...
def make_year_distribution(papers: list[dict]) -> Optional[str]:
    ...
    try:
        ...
        ax.bar(
            [str(y) for y in sorted_years], counts, color="#6366f1", edgecolor="white"
        )
...
def make_plotly_cluster_distribution(
    ...
    try:
        ...
        label_counts = _Counter(int(lbl) for lbl in labels if lbl != -1)
        if not label_counts:
            return None
...
def make_plotly_gap_frequency(gaps: list) -> Optional[dict]:
    ...
    try:
        ...
        titles = [
            "<br>".join(textwrap.wrap(g.gap_title, width=40)) for g in sorted_gaps
        ]
    ...
    try:
        ...
        for i, paper in enumerate(papers):
            ...
            for gap in gaps:
                ...
                full_title_hover = "<br>".join(
                    textwrap.wrap(paper.get("title", ""), width=60)
                )
                hover_row.append(
...
def generate_all_visualizations(
    ...
    return {
        ...
        "metric_frequency": make_frequency_bar(
            pattern.metric_frequency, "Metric Frequency"
        ),
        "plotly_umap": make_plotly_umap(reduced_2d, labels, titles),
        "plotly_metric_freq": make_plotly_frequency_bar(
            pattern.metric_frequency, "Metric Frequency"
        ),
        "plotly_cluster_distribution": make_plotly_cluster_distribution(
            labels, _themes
        ),
