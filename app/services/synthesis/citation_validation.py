...
def validate_gap_against_supported_papers(
    ...
    if not cluster_papers:
        return {
            "status": "unvalidated",
            "reason": "No papers available for validation.",
        }
    ...
    if len(target_text) < 20:
        return {
            "status": "unvalidated",
            "reason": "Gap description too short for semantic validation.",
        }
...
async def verify_gap_with_llm(
    ...
    if not citations or not llm_client:
        return {
            "status": "unvalidated",
            "reason": "Insufficient data for LLM verification.",
        }
...
def validate_gap_citations(
    ...
    cited_numbers = sorted(
        {int(m.group(1)) for m in re.finditer(r"\[(\d+)\]", gap_text)}
    )
    ...
    hallucinated = [
        n for n, s in grounding_scores.items() if s < _HALLUCINATION_THRESHOLD
    ]
    weakly_supported = [
        n
        for n, s in grounding_scores.items()
        if _HALLUCINATION_THRESHOLD <= s < _WEAK_SUPPORT_THRESHOLD
    ]
    well_grounded = [
        n for n, s in grounding_scores.items() if s >= _WEAK_SUPPORT_THRESHOLD
    ]
    ...
    if status == "grounded":
        ...
        reasoning = (
            " ".join(issues)
            if issues
            else "The validation engine identifies several gaps between the claim and the cited evidence."
        )
